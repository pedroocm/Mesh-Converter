...
def hexLaplacianOld2(matrixCells, arrayOfPoints, numberOfCellValues, inactiveCells, pinchArray, porosity, tol = 0.1):
	(nx, ny, nz) = numberOfCellValues
	phi = 0.9
	for i in range(nx):
		for j in range(ny):
			for index in range(4):
				for k in range(nz):
					...
					if pinchArray[cellIndex] == 1 and inactiveCells[cellIndex] == 0:
						continue

					firstCell = matrixCells[i][j][k]
					if not checkThickness(firstCell, arrayOfPoints, tol):
						continue

					topFirstIndex = firstCell[index]
					firstPoint = arrayOfPoints[topFirstIndex]
					upperPoint = firstPoint
					if k > 0:
						upperCell = matrixCells[i][j][k-1]
						if inactiveCells[cellIndex - 1] != 0 and firstPoint == arrayOfPoints[upperCell[index+4]]:
							upperPoint = arrayOfPoints[upperCell[index]]
							
					pinchVertices = [topFirstIndex]
					flux = True
					while(k < nz):
						currCell = matrixCells[i][j][k]
						if not enablesFlux(currCell, cellIndex, arrayOfPoints, inactiveCells, pinchArray, porosity, tol):
							flux = False
							break
						if not checkThickness(currCell, arrayOfPoints, tol):
							break
						pinchVertices.append(currCell[index+4])
						k += 1
						cellIndex += 1
					...
					lastPoint = arrayOfPoints[lastCell[index+4]]
					lowerPoint = lastPoint
					if k < nz:
						lowerCell = matrixCells[i][j][k]
						if inactiveCells[cellIndex] != 0 and lowerPoint == arrayOfPoints[lowerCell[index]]:
							lowerPoint = arrayOfPoints[lowerCell[index+4]]
...
def checkUpperConnectivity(i, j, k, columnIndex, matrixCells, arrayOfPoints):
	if k == 0:
		return True
	cell = matrixCells[i][j][k]
	upperCell = matrixCells[i][j][k-1]
...
def determineFluxCells(matrixCells, numberOfCellValues, arrayOfPoints, qtdCells, inactiveCells, pinchArray, porosity, tol = 0.1):
	...
	#print("qtdFlux =", len(fluxCells))
	#print(fluxCells)
	#print("qtd =", newQtdCells)
	for i in range(nx):
		for j in range(ny):
			k = 0
			while k < nz:
				...
				cellIndex = i*nz*ny + j*nz + k
				debug = cellIndex == 22
				if not enablesFlux(cell, cellIndex, arrayOfPoints, inactiveCells, pinchArray, porosity, tol) or \
					hasSmallThickness(cell, arrayOfPoints, tol):
					fluxCells[cellIndex] = 0
					newQtdCells -= 1
					if inactiveCells[cellIndex] != 0:
						inactiveCells[cellIndex] = 0
						qtdCells -= 1
					k += 1
				else:
					intermediateCells = []
					...
					while k < nz:
						...
						if not enablesFlux(currCell, currCellIndex, arrayOfPoints, inactiveCells, pinchArray, porosity, tol):
					...
					if not flux or k == nz:
						for index in intermediateCells:
							newQtdCells -= 1
							fluxCells[index] = 0
							if inactiveCells[index] != 0:
								inactiveCells[index] = 0
								qtdCells -= 1
						k += 1
						continue
					
	#print("after:",inactiveCells)
...
def transformTo3dListOld(coordValues, zcornValues, numberOfCellValues, inactiveCells = [], pinchArray = [], porosity = []):
...
def transformTo3dList(coordValues, zcornValues, numberOfCellValues, inactiveCells = [], pinchArray = [], porosity = [], permeability = []):
	#Create bidimensional list with line coordinates
		lineCoords = [] # The number of inner brackets is equal to the number of line coordinates in the Y-direction
		lineCoordsTuples = zip(*[iter(coordValues)] * 6) # Each tuple contains 6 numbers (x1, y1, z1, x2, y2, z2). (x1, y1, z1) represents the first point of a coord line; the other 3 number, the second point
		...
		out_pinchArray = [1 for i in range(qtdCelulas)]
		out_porosity = [-1 for i in range(qtdCelulas)]
		out_permeability = [ [-1 for i in range(qtdCelulas)] for j in range(3) ]
		count = 0
		...
		for k in range(nz):
			for j in range(ny):
				for i in range(nx):
					...
					for axis in range(3):
						if permeability[axis] != []:
							out_permeability[axis][i*nz*ny + j*nz + k] = permeability[axis][i + j*nx + k*nx*ny]
		...
		for axis in range(3):
			if permeability[axis] == []:
				out_permeability[axis] = []

		return matrixCells, arrayOfPoints, qtdCelulas, out_inactive, out_pinchArray, out_porosity, out_permeability
...
def transformToMSH2(matrixCells, arrayOfPoints, qtdCelulas, numberOfCellValues, inactiveCells, pinchArray, permeability, porosity, nome):
	...
	with open(nome + ".msh", "w") as newFile:
		...
		for i in range(nx):
			for j in range(ny):
				for k in range(nz):
					...
					if pinchArray != []:
						out_pinchArray.append(pinchArray[i*nz*ny + j*nz + k])
					for l in range(3):
						if(permeability[l] != []):
							out_permeability[l].append(permeability[l][i*nz*ny + j*nz + k])
					if porosity != [] and porosity != "":
						out_porosity.append(porosity[i*nz*ny + j*nz + k])
		...
		labels = ["X", "Y", "Z"]
		for (axisPermeability, axis) in zip(out_permeability, labels):
			if (axisPermeability != []):
				newFile.write(f"\n$ElementData\n1\n\"{axis}-Permeability\"\n1\n0.0\n3\n0\n1\n{qtdCelulas}\n")
				for (i, permeability) in enumerate(axisPermeability):
					newFile.write(f"{i+1} {permeability}\n")
				newFile.write("$EndElementData")

		if (porosity != []):
			newFile.write(f"\n$ElementData\n1\n\"Porosity\"\n1\n0.0\n3\n0\n1\n{qtdCelulas}\n")
			for (i, porosity) in enumerate(out_porosity):
				newFile.write(f"{i+1} {porosity}\n")
			newFile.write("$EndElementData")
